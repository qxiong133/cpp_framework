...
#ifndef __ZMSG_CPP_H_INCLUDED__
#define __ZMSG_CPP_H_INCLUDED__
...
public:
   ...
   bool recv(zmq::socket_t & socket) {
      ...
      while(1) {
         ...
         ustring data;
         data.assign((unsigned char*) message.data(),message.size());
   ...
   const unsigned char * body_data (size_t &len)
   {
       if (m_part_data.size()){
           len = m_part_data[m_part_data.size() - 1].size();
           //unsigned char * data = (unsigned char *) malloc(len)
           //memcpy(data,m_part_data[m_part_data.size() - 1].data(),len);
           //return data;
           return m_part_data[m_part_data.size() - 1].data();
           }
       else
           return 0;
   }
   ...
   // zmsg_append
   void push_back(char *part, size_t len) {
      ustring data;
      data.assign((unsigned char*)part,len);
      m_part_data.push_back(data);
   }
   // zmsg_append
   void push_back(ustring &data) {
      m_part_data.push_back(data);
   }
