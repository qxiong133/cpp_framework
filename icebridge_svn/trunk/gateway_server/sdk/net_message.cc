...
size_t NetMessage::getBytes(int length, iterator buff) {
    ...
    std::copy(this->begin(),this->begin()+length,buff);

    return size_t(length);
}

size_t NetMessage::setBytes(int length, iterator buff) {
	...
	if( length > kDynamicThreshold){
		length = kDynamicThreshold;
	}

	std::copy(buff,buff+length,this->begin());
