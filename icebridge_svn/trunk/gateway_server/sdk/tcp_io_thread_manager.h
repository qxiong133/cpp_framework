...
class TCPIOThreadManager : public boost::noncopyable {
 public:
  ...
  typedef boost::shared_ptr<PlayersManager> PlayerManagerPointer;

 //typedef boost::shared_ptr<SessionFilter> SessionFilterPointer;
  ...
  // returns the main thread.
	TCPIOThread& GetThread() {
		if (threads_.size() == 1)
			return *threads_[kMainThreadID]; // single thread

		if (++next_thread_id_ >= threads_.size())
			next_thread_id_ = 1;
