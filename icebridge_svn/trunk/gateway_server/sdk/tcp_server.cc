...
TCPServer::TCPServer(const tcp::endpoint& endpoint,
    ...
    SessionPointer session(new TCPSession(io_thread_manager_, session_filter_creator_()));//session and thread
...
void TCPServer::HandleAccept(SessionPointer session,
  ...
  io_thread_manager_.OnSessionConnect(session, handler);//connect session and handler
