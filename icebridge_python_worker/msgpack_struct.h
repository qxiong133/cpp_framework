...
struct VersionInforResponse{
	...
	string  update_URL;
	MSGPACK_DEFINE(build_version,arms_version,client_version,update_URL);
};
...
struct ArmsResponse{
	...
	string  magic_skill;
	string  name;
	MSGPACK_DEFINE(type,level,hp,defense,attack,move_speed,fire_rate,need_wood,need_stone,need_iron,need_soil,population,build_time,available,magic_skill,name);
};
...
struct MyAttruibuteResponse{
	...
	string  combat_uuid;
	MSGPACK_DEFINE(player_index,diamond,race,avoid_time,workers,wood,stone,iron,soil,record,level,worldmap_x,worldmap_y,name,city_hall_uuid,my_build,backpack_num,recuite_type,chance_num,build_num,combat_uuid);
};
...
struct HeroResponse{
	...
	int  where;
	HeroBase  base_att;
	MSGPACK_DEFINE(hero_sn,name,hero_uuid,quality,level,leader,strategy,economy,strength,magic_power,leader_grow,strategy_grow,economy_grow,strength_grow,magic_power_grow,cur_hp,max_hp,cur_mp,max_mp,attack,defence,pre_skill,study_skill,cur_goods_space,max_goods_space,exp,type,where,base_att);
};
...
struct msgHeroCastSkillRequest{
	...
	short  skillID;
	MSGPACK_DEFINE(heroID,isAtk,targets,skillID);
};
